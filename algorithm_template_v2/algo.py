...
model = load(client.file("data://#algo_username#/CreditCardApproval/model-b.joblib").getFile().name)

def apply(input):
    
    print("DEBUG with print: age_highest: ", str(input.get("age_highest", 1)))
    logging.info("DEBUG with logging: age_highest: "+str(input.get("age_highest", 1)))
    ...
    client.report_insights({"owns_home": input.get("owns_home", 1), "has_work_phone": input.get("has_work_phone", 0)})
    ...
    #     file_path = "data://algorithmia_se/CreditCardApproval_AAR/adverse_action_reasons_" + \
    #     str(time.time()) + ".json"
